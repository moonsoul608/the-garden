...
-- A publication checkpoint is also the durable idempotency receipt. Existing
-- non-publication checkpoints remain valid with both source fields null.
alter table public.content_versions
  add column source_revision_id uuid,
  add column source_lock_version bigint,
...
comment on column public.content_versions.source_revision_id is
  'Consumed Review revision that produced this publication checkpoint; retained without a foreign key for durable retry receipts.';
comment on column public.content_versions.source_lock_version is
  'Exact optimistic lock consumed by this publication checkpoint.';

create unique index content_versions_publication_receipt_idx
  on public.content_versions (source_revision_id)
  where source_revision_id is not null;

-- Keep Draft identity creation working through the existing SECURITY INVOKER
-- command, while removing every direct authenticated path that can insert,
-- update, or delete a Published projection.
drop policy contents_garden_keeper_all on public.contents;

create policy contents_garden_keeper_read_all
on public.contents
for select
to authenticated
using ((select private.is_garden_keeper()));

create policy contents_garden_keeper_insert_draft
on public.contents
for insert
to authenticated
with check (
...
-- Versions are created only by the publication command. Review consumption is
-- likewise owned by that command, after its immutable version insert succeeds.
revoke insert on table public.content_versions from authenticated;
revoke delete on table public.content_revisions from authenticated;
revoke insert, update, delete on table public.tags from authenticated;
revoke insert, update, delete on table public.content_tags from authenticated;

create function public.publish_review_revision(
  p_content_id uuid,
  p_revision_id uuid,
