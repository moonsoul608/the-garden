...
create table public.v1_migration_imports (
  import_digest text primary key,
  source_digest text not null,
  destination_state_digest text not null,
  source_version jsonb not null,
  result jsonb not null,
  imported_at timestamptz not null default now(),
  constraint v1_migration_imports_import_digest_format check (
    import_digest ~ '^sha256:[a-f0-9]{64}$'
  ),
  constraint v1_migration_imports_source_digest_format check (
    source_digest ~ '^sha256:[a-f0-9]{64}$'
  ),
  constraint v1_migration_imports_destination_digest_format check (
    destination_state_digest ~ '^sha256:[a-f0-9]{64}$'
  ),
  constraint v1_migration_imports_source_version_object check (
    jsonb_typeof(source_version) = 'object'
  ),
  constraint v1_migration_imports_result_object check (
    jsonb_typeof(result) = 'object'
  )
);

comment on table public.v1_migration_imports is
  'Immutable idempotency receipt for the digest-approved V1 import; it is not a public content source.';

revoke all on table public.v1_migration_imports
  from public, anon, authenticated, service_role;
grant select on table public.v1_migration_imports to service_role;

create function public.execute_v1_import(p_payload jsonb)
returns jsonb
language plpgsql
volatile
security definer
...
begin
  ...
  if exists (
    ...
    where nullif(btrim(item->>'legacyId'), '') is null
       ...
       or item->>'growthStage' not in ('Seed', 'Sprout', 'Growing', 'Bloom', 'Dormant')
