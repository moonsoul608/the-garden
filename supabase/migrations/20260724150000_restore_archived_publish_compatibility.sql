...
-- Narrow forward-only repair: allow a restored Review for Archived content to
-- publish through the existing atomic publishing RPC. Draft and Published
-- source behavior remains unchanged.
...
begin
  ...
  if content.lifecycle not in ('Draft', 'Published', 'Archived') then
  ...
  if content.lifecycle = 'Draft' then
    ...
    if content.lifecycle = 'Published' and content.archived_at is not null then
      raise invalid_parameter_value using message = 'invalid_content_state';
    end if;

    if content.lifecycle = 'Archived'
       and (content.archived_at is null or content.archived_by is null) then
      raise invalid_parameter_value using message = 'invalid_content_state';
    end if;
  ...
  begin
    ...
    set
      ...
      archived_at = case
        when projection.lifecycle = 'Archived' then null
        else projection.archived_at
      end,
      archived_by = case
        when projection.lifecycle = 'Archived' then null
        else projection.archived_by
      end,
